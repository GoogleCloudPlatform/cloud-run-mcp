...
// Import ensureAdcCredentials as a named export
import { ensureAdcCredentials } from '../lib/adc-check.js'; // Ensure this path is correct for your project structure
...
async function test(name, testFn) {
  ...
  let passed = true;
  try {
    await testFn(); // Execute the individual test function
  } catch (e) {
...
async function test_success_adc_found() {
  // Configure mockExec's behavior for this specific test
  mockExecError = null;
  mockExecStdout = 'mock-access-token-12345';
  mockExecStderr = '';

  // Call the function under test, passing our mock `exec`
...
async function test_adc_not_found_specific_stderr() {
  ...
  try {
    // Call the function under test, expecting it to call process.exit
...
async function test_command_not_found_enoent() {
  ...
  assert.strictEqual(mockProcessExitCalled, false, 'process.exit should NOT have been called for ENOENT'); // THIS IS THE KEY CHANGE
...
async function test_other_unexpected_exec_errors() {
  ...
  assert.strictEqual(mockProcessExitCalled, false, 'process.exit should NOT have been called for other errors'); // THIS IS THE KEY CHANGE
  assert.ok(caughtError, 'Error was not caught (promise not rejected)');
}
...
async function runAllTests() {
  const testsToRun = [
    ...
    test_adc_not_found_specific_stderr, // Renamed for clarity
  ...
  for (const testFn of testsToRun) {
    ...
    const testPassed = await test(testName, testFn); // Pass the test function for execution
    if (testPassed) {
      passedCount++;
    } else {
      failedCount++;
  ...
  // Exit the process based on test results
  if (failedCount > 0) {
