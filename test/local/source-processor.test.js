...
describe('Source Processor', () => {
  ...
  test('installDependencies logs warn on failure but does not throw', async () => {
    ...
    const warnCall = progressCallback.mock.calls.find(
      (c) => c.arguments[0] && c.arguments[0].level === 'warn'
    );
    assert.ok(warnCall, 'Should have logged a warning');
    assert.ok(
      warnCall.arguments[0].data.includes('npm failed'),
