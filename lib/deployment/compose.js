...
export async function translateCompose(
  ...
  try {
    ...
    if (resourcesConfig) {
      ...
      console.log('Final --resources-config JSON:', configStr);
...
export async function composeVolumes(
  ...
  if (hasBindVolumes || hasNamedVolumes) {
    ...
    const bucketNameCandidate = `${projectNumber}-${sanitizedComposeProjectName}-${region}-compose`;
    ...
    if (hasBindVolumes) {
      ...
      for (const [serviceName, mounts] of entries) {
        ...
        for (const mount of mountsArray) {
          ...
          console.log(`Setting mount_source for service ${serviceName}: ${gcsPath}`);
          ...
          if (stats.isDirectory()) {
            ...
            await uploadDirectory(bucket, fullSourcePath, gcsPath, progressCallback);
          } else {
            await logAndProgress(
  ...
  // Handle named volumes (ensure bucket exists for GCS volumes)
  const namedVolumes = resourcesConfig.volumes && resourcesConfig.volumes.named_volume;
  if (namedVolumes) {
    const namedEntries = Array.isArray(namedVolumes)
      ? namedVolumes.map((v) => [v.name || 'unknown', v])
      : Object.entries(namedVolumes);

    for (const [volName, volInfo] of namedEntries) {
      if (volInfo.gcs && volInfo.gcs.bucket) {
        await logAndProgress(
          `Ensuring GCS bucket ${volInfo.gcs.bucket} for named volume ${volName}...`,
          progressCallback
        );
