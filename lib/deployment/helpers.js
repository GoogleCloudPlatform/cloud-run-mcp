...
function checkIfComposeFileExists(files) {
  ...
  for (const file of files) {
    if (typeof file === 'string') {
      if (composeFiles.includes(path.basename(file).toLowerCase())) {
        return file;
      }
    } else if (typeof file === 'object' && file.filename) {
      if (composeFiles.includes(path.basename(file.filename).toLowerCase())) {
...
/**
 * Extracts the service name from the compose file content.
 * @param {Array<string|Object>} files - Array of file paths or file objects.
 * @param {string} composeFilePath - The path to the compose file.
 * @returns {string|null} - The extracted service name or null.
 */
export function getServiceNameForComposeApp(files, composeFilePath) {
  try {
    let content;
    if (isFolder(files)) {
      ...
      const file = files.find(
        (f) =>
          (typeof f === 'string' && f === composeFilePath) ||
          (typeof f === 'object' && f.filename === composeFilePath)
      );
      if (file) {
        if (typeof file === 'string') {
    ...
    if (content) {
      ...
      if (composeData && composeData.name) {
        return composeData.name;
      }
    }
  } catch (error) {
    // Silently return null, caller can log or ignore
  }
  return `composeApp-${crypto.randomBytes(8).toString('hex')}`;
}
