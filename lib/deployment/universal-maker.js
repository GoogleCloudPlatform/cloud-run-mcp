...
const UM_DOWNLOAD_URLS_BY_ARCH = Object.fromEntries(
...
/**
 * Gets the metadata URL for a given download URL.
 * @param {string} downloadUrl - The download URL.
 * @returns {string} The metadata URL.
 */
function getUniversalMakerMetadataUrl(downloadUrl) {
  return downloadUrl
...
async function ensureUniversalMaker(binDir, progressCallback) {
  ...
  const url = UM_DOWNLOAD_URLS_BY_ARCH[key];
  const metadataUrl = getUniversalMakerMetadataUrl(url);
...
export async function runUniversalMaker(appDir, progressCallback) {
  const binDir = path.join(os.homedir(), TEMP_PATHS.BASE, TEMP_PATHS.BIN_SUBDIR);
