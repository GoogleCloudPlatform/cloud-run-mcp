...
import { readArchive, zipFiles } from '../util/archive.js';
import {
...
const GZIP_FILE_NAME = 'source.tar.gz';
...
const REQUIRED_APIS_FOR_ZIP_DEPLOY = [
  'serviceusage.googleapis.com',
  'iam.googleapis.com',
  'storage.googleapis.com',
  'cloudbuild.googleapis.com',
...
async function deployToCloudRun(
  ...
  const containers = zipSourceCode
    ? [
      {
        image: 'scratch',
        baseImageUri: `${location}-docker.pkg.dev/serverless-runtimes/google-22-full/runtimes/python312`,
        sourceCode: {
  ...
  try {
    ...
    try {
      ...
      console.log('Dry run service config:', dryRunServiceConfig);
      if (exists) {
...
export async function deployZip({
  ...
  if (!zipPath) {
    const errorMsg = 'Error: zipPath is required in the configuration object.';
    await logAndProgress(errorMsg, progressCallback, 'error');
    if (typeof process !== 'undefined' && process.exit) {
      process.exit(1);
    } else {
  ...
  try {
    ...
    await logAndProgress(`Region: ${region}`, progressCallback);
    await logAndProgress(`Service Name: ${serviceName}`, progressCallback);
    await logAndProgress(`Deploying from zip: ${zipPath}`, progressCallback);
    ...
    const zipBuffer = await readArchive(zipPath);
    await uploadToStorageBucket(
      context,
      bucket,
      zipBuffer,
