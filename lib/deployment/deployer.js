...
async function deployWithZip(
  ...
  try {
    ...
    await logAndProgress(`Deployment completed successfully with zip deploy`, progressCallback);
    return service;
  } finally {
...
async function deployWithBuild(
  ...
  await logAndProgress(`Deployment completed successfully with source build deploy`, progressCallback);
  return service;
}
