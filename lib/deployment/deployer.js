...
import {
  ...
  checkIfNodeJsRuntime,
  createNodeJSNoBuildSourceContainer,
} from './helpers.js';
...
export async function deploy({
  ...
  try {
    ...
    const canDoZipDeployment = packageJson?.scripts?.start;
...
async function deployWithZip(
  ...
  await logAndProgress(
    `Node.js runtime detected with start script. Attempting direct source deployment...`,
    progressCallback
  );
  ...
  const archiveName = TARGZ_FILE_NAME;
  ...
  const container = createNodeJSNoBuildSourceContainer({
    bucketName,
...
async function deployWithBuild(
  ...
  const archiveName = ZIP_FILE_NAME;
