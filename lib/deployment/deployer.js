...
import { runCompose as downloadRunCompose, resourceCompose, translateCompose } from './compose.js';
...
export async function deployCompose({
  ...
  try {
    ...
    const resourcesConfig = JSON.parse(resourceOutput);

    // Handle source builds if any
    if (resourcesConfig.source_builds) {
      for (const [serviceName, buildInfo] of Object.entries(
        ...
        const archiveName = `${serviceName}.zip`;
        const bucketName = `${projectId}-cloudrun-sources`;
    ...
    for (const [serviceName, yamlPath] of Object.entries(services)) {
      const fullYamlPath = path.join(folderPath, yamlPath);
      await logAndProgress(
        `Deploying service ${serviceName} using configuration from ${fullYamlPath}...`,
        progressCallback
      );

      const serviceConfig = yaml.load(fs.readFileSync(fullYamlPath, 'utf8'));
      ...
      try {
        ...
        if (error.code === 404) {
          await logAndProgress(
            `Service ${serviceName} does not exist. Creating it...`,
            progressCallback
          );
      ...
      try {
        ...
        await logAndProgress(
          `Failed to set public access for service ${serviceName}: ${error.message}`,
          progressCallback,
          'warn'
        );
      }
      deploymentResults.push(response.data || response);
    }

    return {
      message: `${Object.keys(services).length
        } services deployed from compose file`,
