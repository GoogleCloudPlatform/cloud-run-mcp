...
export async function ensureStorageBucketExists(
  ...
  try {
    ...
    if (exists) {
      ...
      if (String(metadata.projectNumber) !== String(expectedProjectNumber)) {
        if (!isFallback) {
...
export async function uploadToStorageBucket(
  ...
  try {
    ...
    await logAndProgress(
      `File ${destinationBlobName} uploaded successfully to gs://${bucket.name}/${destinationBlobName} (generation: ${metadata.generation}).`,
      progressCallback
    );
    return file;
  } catch (error) {
