...
import { logAndProgress, getProjectNumber } from '../util/helpers.js';
 ...
 * @param {boolean} [isFallback=false] - Whether this is a fallback attempt (avoids infinite recursion).
...
export async function ensureStorageBucketExists(
  ...
  try {
    ...
    if (exists) {
      ...
      let metadata;
      try {
        [metadata] = await callWithRetry(
          () => bucket.getMetadata(),
          `storage.bucket.getMetadata ${bucketName}`
        );
      } catch (metaErr) {
        metadata = {};
      }
      ...
      if (
        ...
        String(metadata.projectNumber) !== String(expectedProjectNumber)
      ) {
        if (!isFallback) {
          const randomSuffix = crypto.randomBytes(4).toString('hex');
          const fallbackBucketName = `${bucketName}-${randomSuffix}`;
          await logAndProgress(
            `Bucket ${bucketName} belongs to project number ${metadata.projectNumber || 'unknown'}, not expected project number ${expectedProjectNumber}. Falling back to randomized bucket ${fallbackBucketName}...`,
            progressCallback,
            'warn'
          );
          ...
          const errorMsg = `Security Error: Fallback bucket ${bucketName} belongs to project number ${metadata.projectNumber || 'unknown'}, not expected project number ${expectedProjectNumber}.`;
      ...
      try {
        ...
        if (!isFallback && createError.message?.includes('already exists')) {
          const randomSuffix = crypto.randomBytes(4).toString('hex');
          const fallbackBucketName = `${bucketName}-${randomSuffix}`;
          await logAndProgress(
            `Failed to create storage bucket ${bucketName} (${createError.message}). Falling back to randomized bucket ${fallbackBucketName}...`,
            progressCallback,
            'warn'
          );
          return await ensureStorageBucketExists(
 ...
 * @param {string} destinationBlobName - The name of the file in the bucket.
 * @param {function(object): void} [progressCallback] - Optional callback for progress updates.
...
export async function uploadToStorageBucket(
  ...
  try {
    ...
    const [metadata] = await callWithRetry(
      () => file.getMetadata(),
      `storage.bucket.file.getMetadata ${destinationBlobName}`
    );
