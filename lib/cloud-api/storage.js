 ...
 * @param {string} [location] - The location to create the bucket in if it doesn't exist. Defaults to 'us'.
...
export async function grantBucketAccess(
  ...
  try {
    ...
    console.error(`Error granting access to bucket ${bucket.name}:`, error);
    await logAndProgress(errorMessage, progressCallback, 'warn');
    // We don't throw here to avoid failing the entire deployment if IAM update fails
    // as the user might have already granted it manually.
  }
}
