 ...
 * @throws {Error} If the API fails to enable or if there's an issue checking its status.
 */
export async function enableApiWithRetry(projectId, api, accessToken, progressCallback) {
  const serviceUsageClient = await getServiceUsageClient(projectId, accessToken);
 ...
 */
export async function ensureApisEnabled(projectId, apis, accessToken, progressCallback) {
