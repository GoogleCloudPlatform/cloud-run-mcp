...
export async function verifyAccessToken(accessToken, audience) {
  try {
    ...
    if (audience && tokenInfo.aud !== audience) {
      throw new Error(
        `Invalid audience: expected ${audience}, got ${tokenInfo.aud}`
      );
    }
...
export async function ensureGCPCredentials() {
  ...
  try {
    ...
    console.error(
      'ERROR: Google Cloud Application Default Credentials are not set up. Checking for OAuth configuration...'
    );

    if (process.env.OAUTH_ENABLED === 'true') {
      console.log('OAuth is enabled via environment variable.');
      return true;
    } else {
      console.error(
        'OAuth is not enabled. Please set up GCP credentials or enable OAuth.'
      );
