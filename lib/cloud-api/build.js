...
export async function triggerCloudBuild(
  ...
  if (hasDockerfile) {
    ...
    const [operation] = await callWithRetry(
      () => cloudBuildClient.createBuild({ projectId, build }),
    ...
    const [getBuildOperation] = await callWithRetry(
      () =>
        cloudBuildClient.getBuild({
          id: buildId,
          projectId,
          // If using location-specific builds, sometimes 'name' is required over id/projectId
          // But v1 usually accepts id/projectId for global/regional builds if configured correctly.
        }),
