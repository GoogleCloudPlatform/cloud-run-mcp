...
export async function triggerCloudBuild(
  ...
  if (serviceAccount) {
    build.serviceAccount = `projects/${projectId}/serviceAccounts/${serviceAccount}`;
  }

  const { ProjectsClient } = await import('@google-cloud/resource-manager');
  const client = new ProjectsClient();
  const [policyBefore] = await client.getIamPolicy({
    resource: `projects/${projectId}`,
  });
  console.log(
    'Current IAM Policy before ensure API:',
    JSON.stringify(policyBefore)
  );
