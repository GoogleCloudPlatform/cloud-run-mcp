...
export async function triggerCloudBuild(
  ...
  if (hasDockerfile) {
    ...
    build.buildpackBuild = {
      image: targetImageUrl, // The destination image
    };
  }
  ...
  const [operation] = await callWithRetry(
    () => cloudBuildClient.createBuild(buildRequest),
    'cloudBuild.createBuild'
  );

  // The build ID is in the metadata of the operation
  const buildId = operation.metadata.build.id;
  await logAndProgress(
    `Cloud Build job ${buildId} started...`,
    progressCallback
  );
