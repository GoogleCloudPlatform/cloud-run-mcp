 ...
 */
export async function isBinaryUpToDate(binPath, artifactParams, accessToken, progressCallback) {
  const { project, location, repository, artifactPath, displayName } = artifactParams;
 ...
 */
export async function ensureRepositoryDownloaded(binPath, artifactParams, accessToken, progressCallback) {
  const { project, location, repository, artifactPath, displayName } = artifactParams;
  const binDir = path.dirname(binPath);

  if (await isBinaryUpToDate(binPath, artifactParams, accessToken, progressCallback)) {
    return binPath;
  }
