...
import { v2 as CloudRunV2Module } from '@google-cloud/run';
...
function initializeClients(projectId) {
  if (!storage) storage = new Storage({ projectId });
  if (!cloudBuildClient) cloudBuildClient = new CloudBuildClient({ projectId });
  if (!artifactRegistryClient) artifactRegistryClient = new ArtifactRegistryClient({ projectId });
  if (!runClient) runClient = new ServicesClient({ projectId });
}
...
async function deployToCloudRun(
          ...
          resources: resources
            ? {
                limits: {
                  ...
                  ...(resources.gpu && { 'nvidia.com/gpu': resources.gpu }),
                },
              }
            : undefined,
        },
      ],
      executionEnvironment: resources?.gpu
  ...
  if (gcsBucket) {
    ...
    service.template.containers[0].volumeMounts = [
      {
        name: 'gcs-volume',
        mountPath: '/models',
      },
...
export async function uploadToStorageBucket(
  ...
  initializeClients(bucket.storage.projectId);
  try {
    await logAndProgress(
...
export async function deployImage({
  ...
  try {
    ...
    await logAndProgress(`Deployment Completed Successfully`, progressCallback);
    progressCallback({ level: 'info', data: `Returning service object from deployImage: ${JSON.stringify(service)}` });
    return service;
  } catch (error) {
    const deployFailedMessage = `Deployment Failed: ${error.message}`;
    console.error(`Deployment Failed`, error);
