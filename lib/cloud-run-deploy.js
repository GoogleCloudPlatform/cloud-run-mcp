...
async function deployToCloudRun(
  ...
  const service = {
    template: {
      ...
      containers: [{ image: imgUrl, resources: {} }],
    },
    labels: {
  ...
  if (gpu) {
    service.template.executionEnvironment = 'EXECUTION_ENVIRONMENT_GEN2';
    limits['nvidia.com/gpu'] = '1';

    // Enforce minimums for GPU, allowing user overrides if they are higher.
    const currentCpu = parseInt(limits.cpu || '0', 10);
    if (currentCpu < 4) {
      limits.cpu = '4';
      if (cpu) {
        // only log if user provided a value that was overridden
        await logAndProgress(
          `CPU overridden to minimum of 4 for GPU support.`,
          progressCallback,
          'warn'
        );
      }
    ...
    // For memory, it's harder to compare units. Let's just set it if not present.
    if (!limits.memory) {
      limits.memory = '16Gi';
    } else {
      await logAndProgress(
        `Warning: When using a GPU, ensure the memory is set to at least 16Gi. The deployment will fail if the provided value '${limits.memory}' is not sufficient.`,
        progressCallback,
        'warn'
      );
    }

    // gpuZonalRedundancy is true if the user wants redundancy.
    // The API flag gpuZonalRedundancyDisabled disables it.
    // So we set the flag to true only when the user wants to disable it.
    if (gpuZonalRedundancy === false) {
      service.template.gpuZonalRedundancyDisabled = true;
      await logAndProgress(
        `GPU zonal redundancy disabled as requested.`,
        progressCallback,
        'info'
      );
    } else {
      // Default is enabled, so no need to set anything explicitly.
      await logAndProgress(
        `GPU zonal redundancy enabled (default).`,
        progressCallback,
        'info'
      );
    }
  ...
  if (Object.keys(limits).length > 0) {
    service.template.containers[0].resources.limits = limits;
  }

  if (port) {
    service.template.containers[0].ports = [{ containerPort: port }];
  }
...
export async function deploy({
  ...
  try {
    ...
    if (gpu) { await logAndProgress(`GPU: ${gpu}`, progressCallback); }
    if (gpuZonalRedundancy) { await logAndProgress(`GPU Zonal Redundancy: ${gpuZonalRedundancy}`, progressCallback); }
