...
async function deployToCloudRun(projectId, location, serviceId, imgUrl, progressCallback, skipIamCheck) {
  ...
  const service = {
    ...
    if (!skipIamCheck) {
      service.invokerIamDisabled = true; // Make public by default, if IAM check is not skipped
    }

  try {
    const exists = await checkCloudRunServiceExists(projectId, location, serviceId, progressCallback);

    // Perform a dry run first, only if IAM checks are not explicitly skipped
    if (!skipIamCheck) {
      try {
      ...
      logAndProgress('Skipping IAM check for deployment as requested.', progressCallback, 'info');
    }
 ...
 */
export async function deploy({ projectId, serviceName = 'app', region = 'us-west1', files, progressCallback, skipIamCheck = false }) {
  if (!projectId) {
