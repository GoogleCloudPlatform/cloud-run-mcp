...
export async function getClient(service, key, loadFn, options = {}, accessToken = null) {
  if (!clients[service].has(key)) {
    const ClientClass = await loadFn();
    const finalOptions = { ...options };
    if (accessToken && accessToken !== 'gcloud_auth') {
      finalOptions.authClient = getAuthClient(options.projectId || 'global', accessToken);
    }
...
export async function getRunClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('run', key, async () => (await import('@google-cloud/run')).v2.ServicesClient, { projectId }, accessToken);
}
...
export async function getBuildsClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('builds', key, async () => (await import('@google-cloud/run')).v2.BuildsClient, { projectId }, accessToken);
}
 ...
 */
export async function getServiceUsageClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('serviceUsage', key, async () => (await import('@google-cloud/service-usage')).ServiceUsageClient, { projectId }, accessToken);
}
...
export async function getStorageClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('storage', key, async () => (await import('@google-cloud/storage')).Storage, { projectId }, accessToken);
}
 ...
 */
export async function getCloudBuildClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('cloudBuild', key, async () => (await import('@google-cloud/cloudbuild')).CloudBuildClient, { projectId }, accessToken);
}
 ...
 */
export async function getArtifactRegistryClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('artifactRegistry', key, async () => (await import('@google-cloud/artifact-registry')).ArtifactRegistryClient, { projectId }, accessToken);
}
...
export async function getLoggingClient(projectId, accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('logging', key, async () => (await import('@google-cloud/logging')).Logging, { projectId }, accessToken);
}
 ...
 */
export async function getBillingClient(projectId = 'global', accessToken = 'gcloud_auth') {
  const key = accessToken !== 'gcloud_auth' ? projectId + accessToken : projectId;
  return getClient('billing', key, async () => (await import('@google-cloud/billing')).CloudBillingClient, {}, accessToken);
}
...
export async function getProjectsClient(accessToken = 'gcloud_auth') {
  ...
  return getClient('projects', key, async () => (await import('@google-cloud/resource-manager')).ProjectsClient, {}, accessToken);
}
