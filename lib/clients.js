...
// Helper to wrap OAuth client for gRPC services which expect a Map for headers
...
export async function getOAuthClient(accessToken) {
  ...
  if (!clients.oauth.has(key)) {
    const client = new OAuth2Client(CLIENT_ID, CLIENT_SECRET, REDIRECT_URI);
    client.setCredentials({ access_token: accessToken });
    clients.oauth.set(key, client);
  }

  return clients.oauth.get(key);
}
