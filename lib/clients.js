...
import { getCloudRunRegions } from './util/helpers.js';
...
/**
 * Gets a Compute Regions Client (from @google-cloud/compute).
 * @param {string} projectId - The Google Cloud project ID.
 * @returns {Promise<import('@google-cloud/compute').RegionsClient>}
 */
export async function getComputeClient(projectId, accessToken = GCLOUD_AUTH) {
  const key = keyGenerator(projectId, accessToken);
  return getClient(
    'compute',
    key,
    async () => (await import('@google-cloud/compute')).RegionsClient,
    { projectId },
    accessToken
  );
}
